...
function initKarmaParallelizer(root, karma, shardIndexInfoMap) {
  ...
  if (
    ...
    console.log(`ID: ${id}`);
    console.log(`Shard Map: ${shardIndexInfoMap ? shardIndexInfoMap[id]: 'No Map'}`);
    console.warn(
  ...
  var fakeContextStatus = createFakeTestContext(root, strategy, shardIndexInfo);
...
function createFakeTestContext(ctx, shouldUseDescription, shardIndexInfo) {
  var depth = 0;
  ...
  function wrap(fn, isFocus, isDescription, isSpec) {
    ...
    return function(name, def) {
      ...
      } else if (isFaking && isDescription) {
  ...
  return {
    beforeStartup: function() {
      ...
      if (!hasOwnSpecs && hasOtherSpecs) {
        console.log(`No tests for shard ${shardIndexInfo.shardIndex}`);
        ctx.describe(
          `[karma-parallel] Add single test to prevent failure (shard ${shardIndexInfo.shardIndex})`,
          function() {

...
function getConfig(fullConfig) {
  ...
  config.customBrowserName =
    'customBrowserName' in config
      ? config.customBrowserName
      : function(browser) {
          return browser.name;
        };
...
function handleBrowserRegister(log, config, browser) {
  ...
  console.log('Aggregated browser id: ' + config.browserIdAlias[browser.name]);
  console.log('Shard Index: ' + config.shardIndexMap[browser.id]);
  log.debug(
...
module.exports = function(/* config */ fullConfig, emitter, logger) {
  ...
  emitter.on('browser_register', (browser) => {
    ...
    browser.name = config.customBrowserName(browser);
    ...
    if (disconnectedBrowser) {
      ...
      Object.keys(config.shardIndexMap)
        .forEach((key) => {
          ...
          for (let i = 0; i < knownBrowsers.length; i++) {
            const sameInstance = knownBrowsers[i].id === key;
            const sameBrowser = knownBrowsers[i].name === browser.name;
            if (sameInstance && !sameBrowser) {
              // only add browser instances to compare list for same browser
              console.log('Browser '+ key + ' skipped.');
              return;
            }
          }

          console.log('Browser ID: '+ key);
          console.log('Shard Index: '+ config.shardIndexMap[key]);
      ...
      if (diff.length !== 0) {
        ...
        console.log('Shard Index: ' + config.shardIndexMap[browser.id]);
        console.log('Current Sharded Indexes: ' + JSON.stringify(currentShardedIndexes));
        console.log('Expected Sharded Indexes: ' + JSON.stringify(expectedShardedIndexes));
        console.log(JSON.stringify(diff));
        console.log('Diff length is ' + diff.length);
        log.debug(
